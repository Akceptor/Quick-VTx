# Quick-VTx
QuickVTx is a handy tool for changing band and channel on drones VTx and backpack-enabled VRx (e.g. Skyzone SteadyView) using touch screen or AUX switches

![image](https://github.com/Akceptor/Quick-VTx/blob/main/docs/Interface.png)

## Features
- Change VTx band and channel using a touch screen interface
- Support Support automatic scanning
- Support for AUX switch control (configurable)
- Support multiple ELRS external modules with separate settings (expeimrimental)
- Save and load settings from persistent storage

## Licensing
For now available to my channel members only (and on request)
Ask in comments here https://youtu.be/NV3nTibY7kM?si=Fd3k8hfj5FR7UyA_ or in private
